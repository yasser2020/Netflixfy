                ...
                <div class="row">
                    ...
                    <p class="movie__description my-2">{{$latest_movie->description}}</p>
                        ...
                        <div class="movie__cta my-4">
                           ...
                           @auth
                           <a href="#" class="btn btn-outline-light text-capitalize"><span class="fa fa-heart movie_fav-icon"></span> add to favorite</a>
                           @else
                        <a href="{{route('login')}}" class="btn btn-outline-light text-capitalize"><span class="fa fa-heart-o movie_fav-icon"></span> add to favorite</a>
                           @endauth
                        </div>
                    </div><!-- end of col -->
           ...
           <div class="movie p-0">
            ...
            <div class="movie__details text-white">
                ...
                <a href="{{route('movies.show',$movie->id)}}" class="btn btn-primary text-capitalize flex-fill mr-2"><i class="fa fa-play"></i> watch now</a>
                    @auth
                <i  class="fa fw {{$movie->is_favored?'fa-heart':'fa-heart-o'}} fa-1x align-self-center movie_fav-icon movie-{{$movie->id}}"
                data-movie-id="{{$movie->id}}"
                data-url="{{route('movies.toggle_favorite',$movie->id)}}"
                    ></i>
                        @else
                <a href="{{route('login')}}" class="text-white align-self-center"><i class="fa fa-heart-o fa-1x align-self-center movie_fav-icon"></i></a>
                    @endauth
                </div>
